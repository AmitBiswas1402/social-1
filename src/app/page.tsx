          ...
          </Link>
          {
            NAVIGATION_ITEMS.map((item) => (
              <Link 
                ...
                <div><item.icon className='mr-6 h-7 w-7' /></div>
                <div>{item.title}</div>
              </Link>
            ))
          }
          <button className='w-60 rounded-full bg-primary px-6 py-4 text-lg text-center hover:bg-blue-400'>
            Post
          </button>
          <button className='w-60 rounded-full flex items-center space-x-2 px-6 py-4 text-lg text-center hover:bg-slate-700'>
            <div className="rounded-full bg-primary w-10 h-10 text-center"><FaUser className="mt-2 mb-2 ml-3" /></div>
            <div className="items-center">
              <div className="text-sm font-bold">Username</div>
              <div className='text-xs font-semibold text-slate-400'>login email</div>
            </div>
            <div><BsThreeDots className="h-6 w-6 ml-10" /></div>
          </button>
        </section>
